...
calc_nondifferentiation <- function(df = df, items, new_var_name, proportion_allowed_missings = 0.1) {
  ...
  df %>%
    ...
    mutate(!!new_var_name := case_when(
      ...
      TRUE ~ {
        ...
        # For combinations of 2 that contain a negative value, set this combination to 0
